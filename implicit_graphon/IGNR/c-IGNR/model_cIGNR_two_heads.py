...
class cIGNR_(nn.Module):
    def __init__(self, net_adj, net_coords, input_card, emb_dim, latent_dim, num_layer, gnn_layers, gnn_type='gin', global_pool='mean', JK='last', drop_ratio=0.,
        ...
        self.modulator_coords = Modulator(
                dim_in = latent_dim,
                dim_hidden = net_coords.dim_hidden,
                num_layers = net_coords.num_layers
            )
        ...
        if self.gnn_type == "srgnn":
            ...
            for layer in range(self.num_layer):
                if self.gnn_type == 'gconvlstm':
                    ...
                    """
                    print()
                    # print(f"GNN layer : {self.gnns[layer]}")
                    # print(f"h_list.shape : {h_list[layer].shape}")
                    print()
                    mlp = getattr(self.gnns[layer], "mlp", getattr(self.gnns[layer], "nn", None))
                    if mlp is None: 
                        continue
                    for j, m in enumerate(mlp):
                        if isinstance(m, nn.Linear):
                            print(f"GIN].MLP[{j}]  {m.in_features} -> {m.out_features}")
                    """
    ...
    def decode_coords(self, z, C_input, coords_true, M, batch, mods = None):
        '''
        z: latent variable [b, latent_dim]. Obtained from graph latent embedding [b, emb_dim], from which to obtain z, [b, latent_dim]
        ...
        for i_b in range(C_input.shape[0]):
            ...
            # print(f"mgrid before shape : {mgrid.shape}")
            mgrid = rearrange(mgrid, 'h w c -> (h w) c')    # [74529, 2]
            ...
            if mods is not None:
                ...
                mods_tmp  = self.modulator_coords(z_tmp) 

            coords_pred = self.net_coords(mgrid[Nb*i_b : Nb*(i_b+1)].to(self.device), mods_tmp) 
            coords = coords_true[Nb*i_b : Nb*(i_b+1)].to(self.device)


            #print(f"coords_true shape : {coords.shape}")
            #print(f"coords_pred shape : {coords_pred.shape}")

            # loss_mse = torch.nn.functional.mse_loss(coords_pred, coords_true)
            loss_mse = torch.nn.functional.smooth_l1_loss(coords_pred, coords)
            loss_b.append(loss_mse)
        ...
        loss_b = torch.stack(loss_b)
        loss   = torch.mean(loss_b) # torch.sqrt(torch.mean(loss_b))

        return loss, coords_pred # ,C_recon_list
